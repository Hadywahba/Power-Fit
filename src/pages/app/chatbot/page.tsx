  ...
  const sendMessageToAPI = async (userMessage: string) => {
    try {
      ...
      if (msg.includes('exercise') || msg.includes('تمرين')) {
        const res = await fetch('/api/exercises'); // غيره باللينك بتاعك
...
Reps: ${data[0].reps}`;
      ...
      if (
        ...
        const res = await fetch('/api/diet'); // غيره
        const data = await res.json();

        if (data?.length) {
          return `🍗 Meal: ${data[0].name}
Calories: ${data[0].calories}`;
      ...
      // 📋 Plans API
      if (msg.includes('plan') || msg.includes('برنامج')) {
        const res = await fetch('/api/plans');
        const data = await res.json();

        if (data?.length) {
          return `📋 Plan: ${data[0].title}`;
        }
        ...
        {messages.map((msg, index) => (
          ...
          >
            <Avatar className="h-10 w-10">
              {msg.sender === 'bot' ? (
                <AvatarImage src="/assets/images/Ellipse 7.png" />
              ) : (
